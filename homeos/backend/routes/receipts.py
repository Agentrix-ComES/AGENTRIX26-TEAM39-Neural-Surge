...
def add_receipt(req: ReceiptRequest):
    ...
    try:
        ...
        for item in valid_items:
            ...
            # Update inventory
            cursor.execute("SELECT quantity FROM inventory WHERE name = ?", (item['name'],))
            row = cursor.fetchone()
            if row:
                try:
                    old_qty = float(row[0])
                    ...
                    cursor.execute("UPDATE inventory SET quantity = ? WHERE name = ?", (str(new_qty), item['name']))
                except ValueError:
                ...
                cursor.execute("""
                    ...
                    VALUES (?, ?, ?, ?)
                """, (item['name'], item['quantity'], item['unit'], expiry_date))
...
def get_inventory():
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute("SELECT id, name, quantity as current_stock, unit, expiry_date FROM inventory")
    rows = cursor.fetchall()
    conn.close()
