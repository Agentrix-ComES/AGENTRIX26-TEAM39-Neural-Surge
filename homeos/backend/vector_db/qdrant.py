...
from llm import get_gemini_embedding
        ...
        if not exists:
            ...
            with open(recipes_file, mode='r', encoding='utf-8') as f:
                ...
                for idx, row in enumerate(reader):
                    ...
                    vector = get_gemini_embedding(text_content, is_query=False)
            ...
            if points:
                ...
                print(f"Successfully indexed {len(points)} recipes into Qdrant collection '{COLLECTION_NAME}' (Gemini text-embedding-004).")
...
def search_recipes_vector(query: str, limit: int = 20):
    ...
    try:
        query_vector = get_gemini_embedding(query, is_query=True)
